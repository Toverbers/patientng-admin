...
export const UseCampaignStore = create((set, get) => ({
	...
	ActivateCampaign: async ({id}) => {
		...
		try {
			const res = await axiosInstance.put(`/activate-crowedFunding/${id}`);
			set({  loading: false });
			console.log("update campaignresult",res.data.message)
			toast.success(res.data.message);
		} catch (error) {
			set({ error: error.response?.data?.message || "Error activating", loading: false });
			console.log(error);
			toast.error(error.response.data.message || "An error occurred");
		}
	},
	finishCampaign: async ({id}) => {
		set({ loading: true });

		try {
			const res = await axiosInstance.put(`/mark-crowedFunding-done/${id}`);
			set({  loading: false });
			console.log("update campaign result",res.data.message)
			toast.success(res.data.message);
		} catch (error) {
			set({ error: error.response?.data?.message || "Error Rounding up campaign", loading: false });
			console.log(error);
			toast.error(error.response.data.message || "An error occurred");
		}
	},

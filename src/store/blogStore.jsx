...
export const UseBlogStore = create((set, get) => ({
    ...
    getBlog: async ({id}) => {
		...
		try {
			...
			const res = await axiosInstance.get(`/get-single-blog-admin/${id}`);
    ...
    changeBlogToDraft: async ({id}) => {
		set({ loading: true });

		try {
			const res = await axiosInstance.put(`/change-to-draft/${id}`);
			set({  loading: false });
			console.log("update blog result",res.data.message)
			toast.success(res.data.message);
		} catch (error) {
    ...
    changeBlogToPublish: async ({id}) => {
		set({ loading: true });

		try {
			const res = await axiosInstance.put(`/change-to-publish/${id}`);
			set({  loading: false });
			console.log("update blog result",res.data.message)
			toast.success(res.data.message);
		} catch (error) {
    ...
    changeBlogToArchive: async ({id}) => {
		set({ loading: true });

		try {
			const res = await axiosInstance.put(`/change-to-archive/${id}`);
			set({  loading: false });
			console.log("update blog result",res.data.message)
			toast.success(res.data.message);
		} catch (error) {
	...
	createCategory: async ({ name}) => {
		...
		try {
			const res = await axiosInstance.post("/post-blog-category", { name });
			set({  loading: false,  });
    ...
    deleteCategory: async ({id}) => {
		...
		try {
			const res = await axiosInstance.delete(`/delete-blog-category/${id}`);
			set({  loading: false });

...
const Advocacy = () => {
  ...
  const filteredData = activeTab === 0 ? advocacyData : advocacyData?.filter(item => item.status === customTabs[activeTab]);
  return (
    <>
     ...
     {loading === true ? <LoadingSpinner /> : <>
      ...
      title="Advocacies" 
    />
        <div className="w-full bg-white rounded-2xl border border-gray-100 overflow-x-auto overflow-y-auto box-border mt-4 py-5">

         <Tabs tabs={customTabs} activeTab={activeTab} onChangeTab={setActiveTab} />
         <div className='flex flex-col space-y-3 p-5'>
          {filteredData?.map((advocacy, index) => ( 

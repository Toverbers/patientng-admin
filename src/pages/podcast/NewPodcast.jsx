...
const NewPodcast = () => {
    const {getPodcastCategory, podcastCategoryData, createPodcast} = UsePodcastStore()
  ...
  const sourceData = [{ source, link }];
    ...
    useEffect(() => {
      ...
      await createPodcast({ title: title, category: category, summary: summary, duration: duration, producedBy: producedBy, source: JSON.stringify(sourceData), image: image, releaseDate: releaseDate, status: "published"})
      }
  ...
  return (
    <div className='px-5 '>
        ...
        <div className=''>
          ...
          <ButtonComponent title="Create new Podcast" buttonStyle="bg-emerald-500 text-white" onClick={handleNewPodcast}/>
            </div>
        </div>
